...
const TanodUpdateForm = ({ items, onItemClick }: Props) => {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await fetch(
        `http://localhost:5000/api/personel4/specific/${items}`
      );
  ...
  const handleSubmits = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5000/api/personel4/update`,
        formData,
        {
          headers: {
...
export default TanodUpdateForm;
